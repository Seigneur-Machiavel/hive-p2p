import { NODE, TRANSPORTS, IDENTITY, DISCOVERY, GOSSIP, UNICAST } from './global_parameters.mjs';
...
export class NodeP2P {
	...
	#onConnect = (peerId, direction) => {
		...
		const dispatchEvents = () => {
			...
			if (DISCOVERY.ON_CONNECT_DISPATCH.SEND_EVENT && !remoteIsPublic) this.broadcast(peerId, 'peer_connected');
		};
	...
	#onDisconnect = (peerId, direction) => {
		...
		const dispatchEvents = () => {
			if (DISCOVERY.ON_DISCONNECT_DISPATCH.SEND_EVENT && !remoteIsPublic) this.broadcast(peerId, 'peer_disconnected');
		};
	 ...
	 * @param {number} [timestamp] default: Date.now() @param {number} [HOPS] default: GOSSIP.HOPS[topic] || GOSSIP.HOPS.default */
