...
export class NodeP2P {
	...
	constructor(id = 'toto', bootstraps = [], init = true) {
		...
		this.peerStore.onSignal.unshift((peerId, data, ws) => this.sendMessage(peerId, 'signal', data, ws));
	...
	setAsPublic(domain = 'localhost', port = VARS.SERVICE_NODE_PORT, upgradeTimeout = VARS.CONNECTION_UPGRADE_TIMEOUT) {
		...
		this.wsServer.on('connection', (ws) => {
			ws.on('message', (message) => {
				...
				if (parsedMessage.type !== 'signal') return console.error('Received message is not a signal');
				this.peerStore.addConnectingPeer(parsedMessage.route[0], ws, parsedMessage.data);
			});
	...
	async #enhanceConnectionLoop(delay = VARS.ENHANCE_CONNECTION_DELAY) {
		...
		while (true) {
			...
			/*if (this.id === 'peer_0') {
				...
				for (const targetId of targets) this.peerStore.addConnectingPeer(targetId);
			}*/

			// test2 -ok
			/*const targets = this.#pickPeerIds(this.opts.targetNeighbors - connectedPeersCount);
			const randomTarget = targets[Math.floor(Math.random() * targets.length)];
			if (randomTarget) this.peerStore.addConnectingPeer(randomTarget);*/
	...
	sendMessage(remoteId, type, data, ws) {
		...
		const routes = this.peerStore.pathFinder.buildRoutes(this.id, remoteId);
		if (!routes.success) return;
		const bestRoute = routes.routes[0].path;
		const msg = new DirectMessage(bestRoute, type, data);
		this.peerStore.sendMessageToPeer(bestRoute[1], msg); // send message to next peer (best route only for now)
	}
	/** @param {string} from @param {DirectMessage} message */
	#handleDirectMessage(from, message) {
	...
	#handleIncomingSignal(senderId, data) {
		const connectingPeer = this.peerStore.peers.connecting[senderId];
		if (!connectingPeer && data.type === 'offer') this.peerStore.addConnectingPeer(senderId, undefined, data);
		else if (data.type === 'answer') this.peerStore.assignConnectingPeerSignal(senderId, data);
	}
